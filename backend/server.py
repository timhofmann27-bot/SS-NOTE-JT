...
request_logger = logging.getLogger("jt.request")
...
JWT_ALGORITHM = "HS256"
ACCESS_TOKEN_MINUTES = 15  # 15 Min — Short-lived access tokens (OWASP A07)
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    """Log every request with method, path, status, duration, and user-id"""
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Try to extract user-id from JWT cookie or Authorization header
        user_id = "anonymous"
        try:
        ...
        request_logger.info(
            ...
            f"{duration_ms:.1f}ms | user={user_id}"
        )
...
async def connect(sid, environ, auth):
    ...
    try:
        ...
        await sio.save_session(
            sid, {"user_id": user_id, "username": user.get("username", "")}
        )
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    """Add security headers to every response — CSP, HSTS, X-Frame-Options, etc."""
...
async def http_exception_handler(request: Request, exc: HTTPException):
    """Handle all HTTPException with consistent error format"""
    details = None
    ...
    request_logger.error(
        f"HTTP {exc.status_code} ({error_code}) on {request.method} {request.url.path}: "
        f"{exc.detail}"
    )
...
async def general_exception_handler(request: Request, exc: Exception):
    """Catch-all for unhandled exceptions — never leak internals to client"""
    request_id = secrets.token_hex(8)
    logger.error(
        f"Unhandled exception [{request_id}] on {request.method} {request.url.path}: "
...
class ProfileUpdate(BaseModel):
    ...
    def validate_name(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            from sanitization import sanitize_display_text
            v = sanitize_display_text(v, max_length=100)
        return v
    ...
    def validate_callsign(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            v = v.strip()
            if not v:
                raise ValueError("Callsign darf nicht leer sein")
            if not re.match(r'^[A-Za-z0-9\-]{1,20}$', v):
                raise ValueError("Callsign: nur A-Z, 0-9, Bindestrich")
        return v

    @field_validator("status_text")
    @classmethod
    def validate_status_text(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            from sanitization import sanitize_display_text
            v = sanitize_display_text(v, max_length=200)
            if not v.strip():
                raise ValueError("Status-Text darf nicht leer sein")
        return v
    ...
    def validate_participants(cls, v: List[str]) -> List[str]:
        for pid in v:
            try:
                ...
                raise ValueError(f"Ungültige Participant-ID: {pid}")
        if len(v) > 50:
            raise ValueError("Maximal 50 Teilnehmer")
        return v
...
class PollCreate(BaseModel):
    ...
    def validate_question(cls, v: str) -> str:
        from sanitization import sanitize_poll_text
        v = sanitize_poll_text(v, max_length=500)
        if not v.strip():
    ...
    def validate_options(cls, v: List[str]) -> List[str]:
        from sanitization import sanitize_poll_text
        cleaned = [sanitize_poll_text(opt.strip(), max_length=200) for opt in v if opt.strip()]
        if len(cleaned) < 2:
...
class StatusUpdate(BaseModel):
    ...
    @field_validator("content")
    @classmethod
    def validate_status_content(cls, v: str) -> str:
        from sanitization import sanitize_display_text
        v = sanitize_display_text(v, max_length=1000)
        if not v.strip():
            raise ValueError("Status-Inhalt darf nicht leer sein")
        return v

    @field_validator("visibility")
    @classmethod
    def validate_visibility(cls, v: str) -> str:
        if v not in ("contacts", "public", "private"):
...
async def create_refresh_token(user_id: str) -> str:
    """Create refresh token AND store its hash in DB for reuse detection"""
    token = jwt.encode(
        {
            "sub": user_id,
        ...
        if reg_attempt and reg_attempt.get("count", 0) > 3:
            ...
            if last:
                ...
                if (datetime.now(timezone.utc) - last).total_seconds() < 60:
                    raise HTTPException(
                        status_code=429,
                        detail="Zu viele Registrierungen. Bitte warte 1 Minute.",
                    )
                else:
...
async def login(input: LoginInput, request: Request, response: Response):
    ...
    # Global IP rate limit: 20 login attempts per IP per 5 minutes (credential stuffing protection)
    global_ip_key = f"ip:{client_ip}"
    global_attempt = await db.login_attempts.find_one({"identifier": global_ip_key})
    if global_attempt and global_attempt.get("count", 0) >= 20:
        last = global_attempt.get("last_attempt")
        if last:
            if last.tzinfo is None:
                last = last.replace(tzinfo=timezone.utc)
            if (datetime.now(timezone.utc) - last).total_seconds() < 300:
                raise HTTPException(
                    status_code=429,
                    detail="Zu viele Anmeldeversuche von dieser IP. Bitte warte 5 Minuten.",
                )
            else:
                await db.login_attempts.delete_one({"identifier": global_ip_key})

    # Track global IP attempt
    await db.login_attempts.find_one_and_update(
        {"identifier": global_ip_key},
        {
            "$inc": {"count": 1},
            "$set": {"last_attempt": datetime.now(timezone.utc)},
            "$setOnInsert": {"locked_until": None},
    ...
    if attempt:
        ...
        if locked_until:
            ...
            if locked_until > now:
                ...
                raise HTTPException(
                    status_code=429,
                    detail=f"Zu viele Fehlversuche. Gesperrt für {remaining} Sekunden.",
                )
            else:
            ...
            raise HTTPException(
                status_code=429,
                detail="Zu viele Fehlversuche. Gesperrt für 900 Sekunden.",
            )
...
async def logout(
    ...
    # Revoke ALL refresh tokens for this user — complete session invalidation
    ...
    await db.users.update_one(
        ...
        {"$set": {"status": "offline", "last_seen": datetime.now(timezone.utc)}},
    )
...
async def change_passkey(
    input: PasskeyChange, request: Request, user: dict = Depends(get_current_user)
):
    ...
    await db.users.update_one(
        {"_id": ObjectId(user["id"])},
        {"$set": {"password_hash": hash_password(input.new_passkey)}},
    )
...
async def update_profile(input: ProfileUpdate, user: dict = Depends(get_current_user)):
    ...
    updated = await db.users.find_one(
        {"_id": ObjectId(user["id"])}, {"password_hash": 0}
    )
...
async def reset_add_code(request: Request, user: dict = Depends(get_current_user)):
    ...
    reset_count = await db.audit_log.count_documents(
        {
            "action": "add_code_reset",
            "user_id": user["id"],
...
async def list_users(user: dict = Depends(get_current_user)):
    ...
    confirmed = await db.contacts.find(
        {"owner_id": user["id"], "status": "confirmed"}
    ).to_list(500)
    result = []
    for c in confirmed:
        u = await db.users.find_one(
            {"_id": ObjectId(c["contact_id"])},
            {"password_hash": 0, "contacts": 0, "blocked_users": 0, "add_me_code": 0},
        )
        if u:
...
async def get_user(user_id: str, user: dict = Depends(get_current_user)):
    ...
    is_contact = await db.contacts.find_one(
        {"owner_id": user["id"], "contact_id": user_id, "status": "confirmed"}
    )
    if not is_contact:
        raise HTTPException(
            status_code=403, detail="Nur bestätigte Kontakte können angesehen werden"
        )
    u = await db.users.find_one(
        {"_id": ObjectId(user_id)},
        {"password_hash": 0, "contacts": 0, "blocked_users": 0, "add_me_code": 0},
...
async def add_by_code(
    input: AddByCodeInput, request: Request, user: dict = Depends(get_current_user)
):
        ...
        if rl and rl.get("count", 0) > 5:
            ...
            if last:
                ...
                if (datetime.now(timezone.utc) - last).total_seconds() < 60:
                    raise HTTPException(
                        status_code=429,
                        detail="Zu viele Versuche. Bitte warte 1 Minute.",
                    )
                else:
    ...
    existing_contact = await db.contacts.find_one(
        {"owner_id": user["id"], "contact_id": target_id, "status": "confirmed"}
    )
    if existing_contact:
...
async def get_contact_requests(user: dict = Depends(get_current_user)):
    ...
    incoming = await db.contact_requests.find(
        {"target_id": user["id"], "status": "pending"}
    ).to_list(100)
    outgoing = await db.contact_requests.find(
...
async def accept_contact_request(
    request_id: str, request: Request, user: dict = Depends(get_current_user)
):
    oid = validate_object_id(request_id)
    req = await db.contact_requests.find_one(
        {"_id": oid, "target_id": user["id"], "status": "pending"}
    )
    if not req:
...
async def get_contacts(user: dict = Depends(get_current_user)):
    ...
    for c in contacts:
        u = await db.users.find_one(
            {"_id": ObjectId(c["contact_id"])}, {"password_hash": 0, "add_me_code": 0}
        )
        if u:
    ...
    if input.is_group:
        ...
        for pid in input.participant_ids:
            if pid != user["id"] and pid not in contact_ids:
                raise HTTPException(
                    status_code=403,
                    detail="Nur Kontakte können zu Gruppen hinzugefügt werden.",
                )
        ...
        existing = await db.chats.find_one(
            {
                "is_group": False,
                "participant_ids": {
                    "$all": [ObjectId(p) for p in all_participants],
                    "$size": 2,
                },
            }
        )
...
async def get_chats(
    ...
    """Chat list with cursor-based pagination — no participant details, no last message content"""
    limit = min(max(limit, 1), 100)

    query = {"participant_ids": ObjectId(user["id"])}

    if cursor:
        try:
            decoded = base64.urlsafe_b64decode(cursor + "==").decode("utf-8")
            last_created_at, last_id = decoded.split("|", 1)
            last_dt = datetime.fromisoformat(last_created_at)
            query["last_message_at"] = {"$lt": last_dt}
        except Exception:
            pass
    ...
    chats = await chats_cursor.to_list(length=limit + 1)
    ...
    if has_more and chats:
        last_chat = chats[-1]
        last_dt = last_chat.get("last_message_at", datetime.now(timezone.utc))
        if last_dt.tzinfo is None:
            last_dt = last_dt.replace(tzinfo=timezone.utc)
        cursor_payload = f"{last_dt.isoformat()}|{str(last_chat['_id'])}"
        next_cursor = base64.urlsafe_b64encode(cursor_payload.encode("utf-8")).decode("utf-8").rstrip("=")
...
async def get_chat(chat_id: str, user: dict = Depends(get_current_user)):
    chat = await db.chats.find_one(
        {"_id": ObjectId(chat_id), "participant_ids": ObjectId(user["id"])}
    )
    if not chat:
...
async def leave_chat(chat_id: str, user: dict = Depends(get_current_user)):
    ...
    chat = await db.chats.find_one(
        {"_id": oid, "participant_ids": ObjectId(user["id"])}
    )
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    if not chat.get("is_group"):
        raise HTTPException(
            status_code=400, detail="Einzelchats können nicht verlassen werden"
        )
    await db.chats.update_one(
        {"_id": oid}, {"$pull": {"participant_ids": ObjectId(user["id"])}}
    )
...
async def add_group_member(
    ...
    if contact_id not in contact_ids:
        raise HTTPException(
            status_code=403, detail="Nur Kontakte können hinzugefügt werden"
        )
    if contact_id in [str(p) for p in chat.get("participant_ids", [])]:
        raise HTTPException(status_code=400, detail="Bereits Mitglied")
    await db.chats.update_one(
        {"_id": oid}, {"$addToSet": {"participant_ids": ObjectId(contact_id)}}
    )
    await ws_emit_to_chat(
        chat_id,
        "chat:member_added",
        {"user_id": user["id"], "added_id": contact_id, "chat_id": chat_id},
    )
...
async def remove_group_member(
    ...
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    if str(chat.get("created_by")) != user["id"] and user["id"] not in chat.get(
        "admin_ids", []
    ):
        raise HTTPException(
            status_code=403, detail="Nur Admins können Mitglieder entfernen"
        )
    body = await request.json()
    ...
    if member_id == user["id"]:
        raise HTTPException(
            status_code=400, detail="Ersteller kann sich nicht selbst entfernen"
        )
    await db.chats.update_one(
        {"_id": oid}, {"$pull": {"participant_ids": ObjectId(member_id)}}
    )
    await ws_emit_to_chat(
        chat_id,
        "chat:member_removed",
        {"user_id": user["id"], "removed_id": member_id, "chat_id": chat_id},
    )
...
async def promote_admin(
    ...
    if str(chat.get("created_by")) != user["id"]:
        raise HTTPException(
            status_code=403, detail="Nur der Ersteller kann Admins ernennen"
        )
    body = await request.json()
    member_id = body.get("member_id")
    if not member_id:
    ...
    if member_id not in [str(p) for p in chat.get("participant_ids", [])]:
        raise HTTPException(
            status_code=400, detail="Nur Mitglieder können Admin werden"
        )
    await db.chats.update_one({"_id": oid}, {"$addToSet": {"admin_ids": member_id}})
    await ws_emit_to_chat(
        chat_id, "chat:admin_promoted", {"user_id": user["id"], "admin_id": member_id}
    )
...
async def demote_admin(
    ...
    if str(chat.get("created_by")) != user["id"]:
        raise HTTPException(
            status_code=403, detail="Nur der Ersteller kann Admins entfernen"
        )
    body = await request.json()
    member_id = body.get("member_id")
    if not member_id:
    ...
    if member_id == user["id"]:
        raise HTTPException(
            status_code=400, detail="Ersteller kann sich nicht selbst degradieren"
        )
    await db.chats.update_one({"_id": oid}, {"$pull": {"admin_ids": member_id}})
    await ws_emit_to_chat(
        chat_id, "chat:admin_demoted", {"user_id": user["id"], "admin_id": member_id}
    )
...
async def update_chat(
    ...
    if str(chat.get("created_by")) != user["id"] and user["id"] not in chat.get(
        ...
        raise HTTPException(
            status_code=403, detail="Nur Admins können Einstellungen ändern"
        )
    body = await request.json()
    updates = {}
    if "name" in body:
        from sanitization import sanitize_display_text
        sanitized_name = sanitize_display_text(body["name"], max_length=100)
    ...
    if "security_level" in body:
        if body["security_level"] not in VALID_SECURITY_LEVELS:
            raise HTTPException(status_code=400, detail="Ungültige Sicherheitsstufe")
...
async def pin_message(
    ...
    await ws_emit_to_chat(
        chat_id, "chat:pinned", {"message_id": message_id, "chat_id": chat_id}
    )
...
async def unpin_message(
    ...
    await db.chats.update_one(
        {"_id": ObjectId(chat_id)},
        {"$unset": {"pinned_message_id": "", "pinned_by": "", "pinned_at": ""}},
    )
...
async def star_message(message_id: str, user: dict = Depends(get_current_user)):
    ...
    if not msg:
        raise HTTPException(status_code=404, detail="Nachricht nicht gefunden")
    chat = await db.chats.find_one(
        {"_id": ObjectId(msg["chat_id"]), "participant_ids": ObjectId(user["id"])}
    )
    if not chat:
...
async def get_starred_messages(chat_id: str, user: dict = Depends(get_current_user)):
    ...
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    messages = (
        await db.messages.find({"chat_id": chat_id, "starred_by": user["id"]})
        .sort("created_at", -1)
        .to_list(100)
    )
...
async def search_messages(chat_id: str, q: str, user: dict = Depends(get_current_user)):
    """Search messages in a chat (sanitized query to prevent NoSQL injection)"""
    chat = await db.chats.find_one(
        {"_id": ObjectId(chat_id), "participant_ids": ObjectId(user["id"])}
    )
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    from sanitization import sanitize_search_query
    safe_query = sanitize_search_query(q, max_length=200)
    query = {"chat_id": chat_id, "content": {"$regex": safe_query, "$options": "i"}}
    messages = (
        await db.messages.find(query).sort("created_at", -1).limit(50).to_list(50)
    )
    return {"messages": [serialize_message(m) for m in messages], "query": html.escape(q)}
...
async def forward_message(
    ...
    target_chat = await db.chats.find_one(
        {"_id": ObjectId(target_chat_id), "participant_ids": ObjectId(user["id"])}
    )
    if not target_chat:
...
async def export_chat(chat_id: str, user: dict = Depends(get_current_user)):
    ...
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    messages = (
        await db.messages.find({"chat_id": chat_id})
        .sort("created_at", 1)
        .to_list(10000)
    )
    export_data = {
...
async def send_message(input: MessageSend, user: dict = Depends(get_current_user)):
    chat = await db.chats.find_one(
        {"_id": ObjectId(input.chat_id), "participant_ids": ObjectId(user["id"])}
    )
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    now = datetime.now(timezone.utc)
    ...
    if chat.get("is_group"):
        ...
        if found_usernames:
            ...
            mentioned_users = await db.users.find(
                {"username": {"$in": [u.lower() for u in found_usernames]}}
            ).to_list(100)
    ...
    msg_doc = {
        ...
        "self_destruct_at": (
            (now + timedelta(seconds=input.self_destruct_seconds))
            if input.self_destruct_seconds
            else None
        ),
...
async def get_messages(
    ...
    await db.messages.delete_many(
        {"chat_id": chat_id, "self_destruct_at": {"$lt": now, "$ne": None}}
    )
    messages = (
        await db.messages.find(query).sort("created_at", -1).limit(limit).to_list(limit)
    )
    messages.reverse()
    msg_ids = [
        m["_id"] for m in messages if user["id"] not in m.get("delivered_to", [])
    ]
    if msg_ids:
        await db.messages.update_many(
            {"_id": {"$in": msg_ids}},
            {
                "$addToSet": {"delivered_to": user["id"]},
...
async def mark_read(input: MessageAck, user: dict = Depends(get_current_user)):
    if len(input.message_ids) > 50:
        raise HTTPException(
            status_code=400, detail="Maximal 50 Nachrichten pro Request"
        )
    for mid in input.message_ids:
        ...
        await db.messages.update_one(
            {"_id": oid}, {"$addToSet": {"read_by": user["id"]}}
        )
...
async def delete_message(
    ...
    if msg["sender_id"] != user["id"]:
        raise HTTPException(
            status_code=403, detail="Nur eigene Nachrichten können gelöscht werden"
        )
    ...
    if age_hours < 24:
        ...
        await db.messages.update_one(
            {"_id": oid}, {"$set": {"content": "[Nachricht gelöscht]", "deleted": True}}
        )
...
class MessageEdit(BaseModel):
    ...
    @field_validator("content")
    @classmethod
    def validate_edit_content(cls, v: str) -> str:
        from sanitization import sanitize_message_content
        v = sanitize_message_content(v, max_length=10000)
        if not v.strip():
            raise ValueError("Nachricht darf nicht leer sein")
        return v
...
async def edit_message(
    ...
    if msg["sender_id"] != user["id"]:
        raise HTTPException(
            status_code=403, detail="Nur eigene Nachrichten können bearbeitet werden"
        )
    age_min = (datetime.now(timezone.utc) - msg["created_at"]).total_seconds() / 60
    if age_min > 15:
        raise HTTPException(
            status_code=400,
            detail="Nachricht kann nur innerhalb von 15 Minuten bearbeitet werden",
        )
    await db.messages.update_one(
        {"_id": oid},
        {
    ...
    await ws_emit_to_chat(
        msg["chat_id"],
        "message:edited",
        {"message_id": message_id, "content": input.content, "chat_id": msg["chat_id"]},
    )
...
async def add_reaction(
    ...
    await ws_emit_to_chat(
        msg["chat_id"],
        "message:reaction",
        {"message_id": message_id, "reactions": reactions, "chat_id": msg["chat_id"]},
    )
...
async def poll_messages(
    chat_id: str, after: Optional[str] = None, user: dict = Depends(get_current_user)
):
    oid = validate_object_id(chat_id)
    chat = await db.chats.find_one(
        {"_id": oid, "participant_ids": validate_object_id(user["id"])}
    )
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    now = datetime.now(timezone.utc)
...
async def get_my_mentions(user: dict = Depends(get_current_user)):
    ...
    messages = (
        await db.messages.find({"mentions": user["id"]})
        .sort("created_at", -1)
        .to_list(100)
    )
    return {"messages": [serialize_message(m) for m in messages]}
...
async def get_unread_mentions_count(user: dict = Depends(get_current_user)):
    ...
    count = await db.messages.count_documents(
        {"mentions": user["id"], "read_by": {"$nin": [user["id"]]}}
    )
...
async def poll_chat_updates(user: dict = Depends(get_current_user)):
    chats = (
        await db.chats.find({"participant_ids": ObjectId(user["id"])})
    ...
    for chat in chats:
        ...
        for pid in chat.get("participant_ids", []):
            p = await db.users.find_one(
                {"_id": pid}, {"password_hash": 0, "contacts": 0, "blocked_users": 0}
            )
            if p:
                participants.append(serialize_user_public(p))
        chat_data["participants"] = participants
...
async def create_poll(
    chat_id: str, input: PollCreate, user: dict = Depends(get_current_user)
):
...
async def list_polls(chat_id: str, user: dict = Depends(get_current_user)):
    ...
    if not chat:
        raise HTTPException(status_code=404, detail="Chat nicht gefunden")
    polls = (
        await db.messages.find({"chat_id": chat_id, "message_type": "poll"})
        .sort("created_at", -1)
        .to_list(100)
    )
...
async def vote_poll(
    poll_id: str, input: PollVote, user: dict = Depends(get_current_user)
):
...
async def get_typing(chat_id: str, user: dict = Depends(get_current_user)):
    ...
    typers = await db.typing.find(
        {"chat_id": chat_id, "user_id": {"$ne": user["id"]}, "at": {"$gt": cutoff}}
    ).to_list(10)
...
async def health():
    ...
    try:
        health_data["connected_users"] = sum(
            len(sids) for sids in connected_users.values()
        )
...
async def public_health():
    """Public health endpoint (no auth, minimal info) — for load balancers"""
    return {
        "status": "ok",
        "service": "SS-Note",
        "timestamp": datetime.now(timezone.utc).isoformat(),
...
async def refresh_token(request: Request, response: Response):
    """Issue new access token using refresh token with ROTATION and REUSE DETECTION"""
    ...
    try:
        ...
        # REUSE DETECTION: Check if this token is still the active one
        ...
        if not active_token:
            ...
            await audit_log("token_reuse_detected", user_id, {
                "detail": "Refresh token reuse detected — possible token theft"
            })
            raise HTTPException(
                status_code=401,
                detail="Sicherheitsverletzung erkannt. Bitte erneut anmelden."
            )
...
async def delete_account(
    ...
    await db.chats.update_many(
        {"participant_ids": ObjectId(uid)},
        {"$pull": {"participant_ids": ObjectId(uid)}},
    )
    ...
    await db.refresh_tokens.update_many({"user_id": uid}, {"$set": {"is_revoked": True}})
...
async def verify_magic_token(request: Request, response: Response):
    ...
    if not doc:
        raise HTTPException(
            status_code=401, detail="Ungültiger oder abgelaufener Token"
        )
...
async def magic_qr_poll(sid, data):
    ...
    if not token:
        return
    doc = await db.magic_tokens.find_one(
        {"token": token, "user_id": session["user_id"]}
    )
...
async def upload_public_key(
    input: PublicKeyUpload, request: Request, user: dict = Depends(get_current_user)
):
...
async def upload_prekeys(
    input: PrekeyBundleUpload, request: Request, user: dict = Depends(get_current_user)
):
    ...
    for otp in input.one_time_prekeys:
        if not BASE64_REGEX.match(otp.get("key", "")):
            raise HTTPException(
                status_code=400, detail=f"Ungültiger One-Time Prekey: {otp.get('id')}"
            )
    ...
    if input.one_time_prekeys:
        existing_otps = await db.prekeys.find(
...
async def get_prekey_bundle(user_id: str, user: dict = Depends(get_current_user)):
    ...
    otps = (
        await db.prekeys.find({"user_id": user_id, "type": "otp"})
        .sort("created_at", 1)
        .to_list(10)
    )
...
async def consume_prekey(
    user_id: str, otp_id: str, user: dict = Depends(get_current_user)
):
...
async def register_push_token(
    input: PushTokenUpload, request: Request, user: dict = Depends(get_current_user)
):
...
async def send_encrypted_message(
    input: EncryptedMessageSend, user: dict = Depends(get_current_user)
):
    ...
    if input.media_ciphertext and not BASE64_REGEX.match(input.media_ciphertext):
        raise HTTPException(
            status_code=400, detail="Ungültige verschlüsselte Medien-Daten"
        )
    ...
    other_participants = [
        pid for pid in chat.get("participant_ids", []) if str(pid) != user["id"]
    ]
...
class PairwiseEncryptedMessageSend(BaseModel):
    ...
    recipients: List[
        dict
    ]  # [{user_id, ciphertext, nonce, dh_public?, msg_num?, media_ciphertext?, media_nonce?}]
...
async def send_pairwise_encrypted_message(
    input: PairwiseEncryptedMessageSend, user: dict = Depends(get_current_user)
):
    ...
    for r in input.recipients:
        if not BASE64_REGEX.match(r.get("ciphertext", "")) or not BASE64_REGEX.match(
            r.get("nonce", "")
        ):
            raise HTTPException(
                status_code=400,
                detail=f"Ungültige verschlüsselte Daten für {r.get('user_id')}",
            )
...
async def create_status_update(
    input: StatusUpdate, request: Request, user: dict = Depends(get_current_user)
):
...
async def delete_status_update(
    status_id: str, request: Request, user: dict = Depends(get_current_user)
):
...
async def shutdown():
    """Graceful shutdown: close MongoDB connections and disconnect all WebSocket clients"""
    logger.info("Shutting down — closing MongoDB connections...")
    try:
        # Disconnect all WebSocket users gracefully
