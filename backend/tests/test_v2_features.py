...
class TestV2Features:
    ...
    def test_generate_username(self, api_client, base_url):
        ...
        for i in range(5):
            ...
            assert (
                len(parts) == 2
            ), f"Username should have format animal-hex: {username}"
        ...
        assert (
            len(usernames) >= 3
        ), f"Generated usernames not random enough: {usernames}"
        print(
            f"✓ Username generator working, generated {len(usernames)} unique usernames"
        )
    ...
    def test_account_deletion_dsgvo(self, api_client, base_url):
        ...
        login_response = api_client.post(
            f"{base_url}/api/auth/login",
            json={"username": test_user["username"], "passkey": test_user["passkey"]},
        )

        assert (
            login_response.status_code == 401
        ), f"User should not be able to login after deletion, got {login_response.status_code}"
    ...
    def test_security_headers(self, api_client, base_url):
        ...
        for header, expected_value in required_headers.items():
            ...
            if not actual_value:
                ...
                print(
                    f"⚠ Header mismatch: {header} = {actual_value} (expected: {expected_value})"
                )
            else:
    ...
    def test_passkey_change_integration(self, api_client, base_url):
        ...
        api_client.headers.update({"Authorization": f"Bearer {token}"})
        wrong_response = api_client.post(
            f"{base_url}/api/auth/change-passkey",
            json={"old_passkey": "WrongOldPass!", "new_passkey": "NewPass456!"},
        )
        ...
        old_login = api_client.post(
            f"{base_url}/api/auth/login",
            json={"username": test_user["username"], "passkey": "OldPass123!"},
        )
        ...
        new_login = api_client.post(
            f"{base_url}/api/auth/login",
            json={"username": test_user["username"], "passkey": "NewPass456!"},
        )
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                if "exp" in payload and "iat" in payload:
                    ...
                    print(
                        f"Token lifetime: {lifetime_hours:.1f} hours ({lifetime_seconds} seconds)"
                    )

                    # Should be 1 hour (3600 seconds), allow small variance
                    assert (
                        3500 <= lifetime_seconds <= 3700
                    ), f"Token lifetime should be ~1h, got {lifetime_hours:.1f}h"
