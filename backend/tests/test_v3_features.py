...
class TestV3Features:
    ...
    def test_magic_qr_creation(self, api_client, base_url, admin_token):
        ...
        assert qr_base64.startswith(
            "data:image/png;base64,"
        ), f"QR should be base64 PNG, got: {qr_base64[:50]}"

        # Verify token is URL-safe base64 (32 bytes = 43 chars)
        assert (
            len(magic_token) >= 40
        ), f"Magic token too short: {len(magic_token)} chars"
    ...
    def test_magic_qr_verify_success(self, api_client, base_url, admin_token):
        ...
        verify_response = api_client.post(
            f"{base_url}/api/auth/magic-verify", json={"token": magic_token}
        )
        print(f"Verify Status: {verify_response.status_code}")

        assert (
            verify_response.status_code == 200
        ), f"Expected 200, got {verify_response.status_code}: {verify_response.text}"
    ...
    def test_magic_qr_verify_invalid_token(self, api_client, base_url):
        ...
        response = api_client.post(
            f"{base_url}/api/auth/magic-verify", json={"token": fake_token}
        )
    ...
    def test_magic_qr_verify_reuse_prevention(self, api_client, base_url, admin_token):
        ...
        assert (
            second_verify.status_code == 401
        ), f"Expected 401 on reuse, got {second_verify.status_code}"
    ...
    def test_refresh_token_rotation(self, api_client, base_url):
        ...
        if new_refresh_token:
            ...
            assert (
                old_token_response.status_code == 401
    ...
    def test_security_headers_all_endpoints(self, api_client, base_url):
        ...
        endpoints = [
            ...
            (
                "POST",
                f"{base_url}/api/auth/login",
                {"username": "wolf-1", "passkey": "Funk2024!"},
                "Auth endpoint",
            ),
        ]
    ...
    def test_app_icon_metadata(self, api_client, base_url):
        ...
        print(
            "✓ App icon update (Jägertruppe Berlin-Brandenburg) - frontend asset, not API testable"
        )
